...
- Webkit autofill (жёлтый фон браузера) решён глобально в `styles.scss` через `transition: background-color 5000s ease-in-out 0s` — не дублируй в компонентах
...
- File suffixes: _.component.ts, _.service.ts, _.directive.ts, _.pipe.ts, \*.token.ts
- Path aliases: @app/_, @core/_, @shared/_, @store/_, @pages/_, @mock-api/_
...
└── src/
    ...
    └── app/
        ...
        │   │   ├── http/              # HttpService (базовый HTTP-клиент)
        │   │   ├── ws/                # WsService (WebSocket)
        │   │   ├── interceptors/      # http-error.interceptor
        │   │   └── controllers/       # REST-контроллеры по доменам
        │   │       ├── chat/          # ChatService, ChatApiService, mappers, dtos
        │   │       └── user/          # UserApiService, mappers
        │   └── environment/           # ApplicationEnvironmentToken
        │
        ├── store/                     # Глобальное состояние (@store/* alias)
        │   ├── app/
        │   ├── auth/
        │   ├── chat/                  # ChatStore
        │   └── user/                  # UserStore
        ...
        │   ├── no-chat/               # / (пустой экран, нет активного чата)
        │   ├── chat/                  # /chat/:id
        ...
        │           ├── components/form-field/  # FormFieldComponent
        │           ├── directives/    # prefix/, suffix/
        │           ├── tokens/        # FORM_FIELD, FORM_FIELD_CONTROL, UI_KIT_ERROR_MESSAGES
        │           ├── utils/         # wrapValidator(validatorFn, customKey)
        │           └── index.ts
```
...
- **FormFieldComponent** — контейнер: floating/static label, error/hint subscript, prefix/suffix slots
  - `label` опционален (default `''`); пустой — label-элемент не рендерится
  - `isFloatingLabel=false` (default) — label над wrapper; `true` — label внутри infix (floating animation)
  - Subscript рендерится только при наличии `hint()` или `control.ngControl`; без них пространство не резервируется
- **UiKitInputDirective** — вешается на нативный `<input>`, реализует FORM_FIELD_CONTROL, регистрируется в FormField через DI
  - Внутри FormField автоматически получает класс `.ui-kit-input--in-field` — убирает фон/border/radius; не переопределяй это в компонентах
- **ErrorTextPipe** — резолвит ключ ошибки в текст: override map → DI map (UI_KIT_ERROR_MESSAGES) → fallback key; находится в `shared/ui-kit/cdk/error-text/`
...
```
Pages → Store → Services → HttpClient + authInterceptor → MockApiInterceptor → db.json
                         → WsService (WebSocket) → mock-api
```
