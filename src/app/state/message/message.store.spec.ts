import { provideHttpClient } from '@angular/common/http';
import { HttpTestingController, provideHttpClientTesting } from '@angular/common/http/testing';
import { TestBed } from '@angular/core/testing';
import { IMessageDto } from '@shared/dtos/message-dto.interface';

import { MessageStore } from './message.store';

const mockMessage: IMessageDto = {
  id: 'msg-1',
  chatId: 'c1',
  senderId: 'u2',
  text: 'Hello',
  createdAt: '2026-04-10T10:00:00.000Z',
};

describe('MessageStore', () => {
  let store: InstanceType<typeof MessageStore>;
  let httpTesting: HttpTestingController;

  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [MessageStore, provideHttpClient(), provideHttpClientTesting()],
    });
    store = TestBed.inject(MessageStore);
    httpTesting = TestBed.inject(HttpTestingController);
  });

  afterEach(() => httpTesting.verify());

  describe('initial state', () => {
    it('should have empty messages and typing maps', () => {
      expect(store.messagesByChatId()).toEqual({});
      expect(store.typingByChatId()).toEqual({});
      expect(store.isLoading()).toBe(false);
    });
  });

  describe('loadMessages', () => {
    it('should load messages for the given chatId', () => {
      store.loadMessages('c1');
      httpTesting.expectOne('/api/chats/c1/messages').flush([mockMessage]);
      expect(store.messagesByChatId()['c1']).toEqual([mockMessage]);
      expect(store.isLoading()).toBe(false);
    });

    it('should set isLoading false on error', () => {
      store.loadMessages('c1');
      httpTesting.expectOne('/api/chats/c1/messages').flush('error', { status: 500, statusText: 'Server Error' });
      expect(store.isLoading()).toBe(false);
    });
  });

  describe('addMessage', () => {
    it('should append message to existing chat messages', () => {
      store.loadMessages('c1');
      httpTesting.expectOne('/api/chats/c1/messages').flush([mockMessage]);

      const newMessage: IMessageDto = {
        id: 'msg-2',
        chatId: 'c1',
        senderId: 'u1',
        text: 'Reply',
        createdAt: '2026-04-10T10:01:00.000Z',
      };

      store.addMessage('c1', newMessage);

      expect(store.messagesByChatId()['c1']).toHaveLength(2);
      expect(store.messagesByChatId()['c1'][1]).toEqual(newMessage);
    });

    it('should create messages array for new chatId', () => {
      const message: IMessageDto = { ...mockMessage, chatId: 'c2' };
      store.addMessage('c2', message);
      expect(store.messagesByChatId()['c2']).toEqual([message]);
    });
  });

  describe('setTyping', () => {
    it('should add userId to typing list when isTyping is true', () => {
      store.setTyping('c1', 'u2', true);
      expect(store.typingByChatId()['c1']).toContain('u2');
    });

    it('should remove userId from typing list when isTyping is false', () => {
      store.setTyping('c1', 'u2', true);
      store.setTyping('c1', 'u2', false);
      expect(store.typingByChatId()['c1']).not.toContain('u2');
    });

    it('should not duplicate userId in typing list', () => {
      store.setTyping('c1', 'u2', true);
      store.setTyping('c1', 'u2', true);
      expect(store.typingByChatId()['c1'].filter((id: string) => id === 'u2')).toHaveLength(1);
    });
  });
});
